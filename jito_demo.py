...
async def main():
    for i in range(10):
        ...
        async with AsyncClientFactory() as rpc_client:
            response = (await rpc_client.get_latest_blockhash(Processed)).value
            print("1",response)
            response = (await rpc_client.get_latest_blockhash(Confirmed)).value
            print("2",response)
            response = (await rpc_client.get_latest_blockhash(Finalized)).value
            print("3",response)
