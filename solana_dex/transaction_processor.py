...
class TransactionProcessor:
    ...
    @staticmethod
    async def _confirm_transaction(swap, txn_signature, pubkey, base_mint):
        status = await swap.confirm_transaction(txn_signature)
        # 上链失败等于出售失败
        if status:
            logger.info(f"{txn_signature} 上链成功")
            await create_task_log(pubkey, base_mint, "", "上链完成", 1, 3, txn_signature)
            return True, "创建任务成功 上链成功"
        else:
            logger.error(f"{txn_signature} 上链失败")
            await create_task_log(pubkey, base_mint, "", "上链失败", 0, 3, txn_signature)
    ...
    async def _web_transaction(base_mint: str, amount, is_buy: bool):
        ...
        async with AsyncClientFactory() as client:
            ...
            if not is_buy:
                ...
                txn_signature = await swap.sell(pool_info.baseMint, amount, ui_amount)
            else:
                txn_signature = await swap.buy(pool_info.baseMint, amount)
            if not txn_signature:
                return False, "创建任务失败"
            asyncio.create_task(
                TransactionProcessor._confirm_transaction(swap, txn_signature, wallet.pubkey, pool_info.baseMint))
        return True, "创建任务完成"
    ...
    async def web_clone_account():
        async with AsyncClientFactory() as client:
            ...
            if not txn_signature:
                return False, "任务创建失败，无法清空余额为空的账户"
            return True, "任务创建成功，已清空余额为空的账户"
