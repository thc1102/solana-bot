...
class Wallet:
    ...
    async def update_token_accounts(self, commitment: Optional[Commitment] = Confirmed):
        try:
            async with AsyncClientFactory() as client:
    ...
    def get_token_accounts_balance(self, mint):
        data = self.get_token_accounts(mint)
        if data is None:
            return None
