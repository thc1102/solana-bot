...
from solana_dex.solana_tx_utils.make_swap_instruction import make_swap_instruction
...
class SwapTransactionBuilder:
    ...
    async def append_get_pool_data(self):
    ...
    async def append_sell(self, amount_in: int, amount_out: int):
        # compute budget
        ...
        await self.append_create_associated_token_account(self.quoteMint)
        # swap
        await self.append_swap(amount_in, source, dest, amount_out)
        # close the account
        await self.append_close_account(dest)
    ...
    async def append_buy(
        ...
        # initialize account
        await self.append_initialize_account(source)
        # open destination account if not exists
        if check_associated_token_account_exists:
            await self.append_if_not_exists_create_associated_token_account(self.mint)
        # swap
        dest = get_associated_token_address(self.payer.pubkey(), self.mint)
        await self.append_swap(amount_in, source, dest, amount_out)
        # close the account
        await self.append_close_account(source)

    async def compile_versioned_transaction(self):
        recent_blockhash = await client_wrapper.get_latest_blockhash(self.client)
    ...
    async def append_set_compute_budget(self, unit_price: int, unit_limit: int):
    ...
    async def append_close_account(self, account: Pubkey):
        self.instructions.append(
            close_account(
                CloseAccountParams(
                    account=account,
    ...
    async def append_create_associated_token_account(self, mint: Pubkey):
        self.instructions.append(
            create_associated_token_account(
    ...
    async def append_create_account_with_seed(self, lamports: int):
        # create account with seed
    ...
    async def append_initialize_account(self, source: Pubkey):
        # initialize account
        self.instructions.append(
